...
def format_chat_history(chat_history: List[Message], user_input=None):
    messages = [
        ("user: " + message.content if isinstance(message, HumanMessage) else "ai: " + message.content)
...
class StateExtractor:
    ...
    lc_gpt_4: ChatOpenAI = ChatOpenAI(model_name="gpt-4")  # type: ignore
    lc_gpt_turbo: ChatOpenAI = ChatOpenAI(model_name="gpt-3.5-turbo")  # type: ignore
    system_state_commentary: SystemMessagePromptTemplate = SystemMessagePromptTemplate(prompt=SYSTEM_STATE_COMMENTARY)  # type: ignore
    system_state_labeling: SystemMessagePromptTemplate = SystemMessagePromptTemplate(prompt=SYSTEM_STATE_LABELING)  # type: ignore
    system_state_check: SystemMessagePromptTemplate = SystemMessagePromptTemplate(prompt=SYSTEM_STATE_CHECK)  # type: ignore
    ...
    async def generate_state_commentary(
        ...
        # format prompt
        state_commentary = ChatPromptTemplate.from_messages([cls.system_state_commentary])
        # LCEL
